...
class VicPinkyNavAdapter(Node):
    ...
    def _execute_callback(self, goal_handle) -> RunTask.Result:
        ...
        if target.start_delay_sec > 0.0:
            self._publish_feedback(
                goal_handle,
                phase='waiting_map_settle',
                progress=0.02,
                detail=(
                    'Waiting '
                    f'{target.start_delay_sec:.1f}s before sending Nav2 goal'
                ),
            )
            self.get_logger().info(
                f'Waiting {target.start_delay_sec:.1f}s before Nav2 goal: '
                f'{goal.target_name}'
            )
            if not self._wait_for_start_delay(
                goal_handle,
                target.start_delay_sec,
            ):
    ...
    def _target_from_goal(self, goal: RunTask.Goal) -> NavigationTarget:
        ...
        try:
            start_delay_sec = float(payload.get('start_delay_sec', 0.0))
        except (TypeError, ValueError) as exc:
            raise ValueError(
                'extra_json.start_delay_sec must be a finite number'
            ) from exc

        if not frame_id:
            raise ValueError('extra_json.pose.frame_id cannot be empty')
        if not math.isfinite(start_delay_sec) or start_delay_sec < 0.0:
            raise ValueError(
                'extra_json.start_delay_sec must be a finite, '
                'non-negative number'
            )
    ...
    @staticmethod
    def _wait_for_start_delay(goal_handle, delay_sec: float) -> bool:
        """Wait before navigation while remaining responsive to cancellation."""
