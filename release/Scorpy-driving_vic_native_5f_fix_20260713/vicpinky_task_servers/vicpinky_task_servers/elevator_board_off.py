...
class ElevatorServers(Node):
    ...
    def exit_target_cm(goal):
        """Read a landing-marker exit distance without changing boarding."""
        ...
        if not math.isfinite(target_cm) or not 5.0 <= target_cm <= 300.0:
            raise ValueError(
                'exit_target_distance_cm must be in [5, 300] cm'
            )
        return target_cm
