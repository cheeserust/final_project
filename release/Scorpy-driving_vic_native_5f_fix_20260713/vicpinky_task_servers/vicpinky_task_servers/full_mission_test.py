...
class FullMissionTest(Node):
    ...
    def take_elevator(self, from_floor, to_floor):
        ...
        if not self.call(self.floor, 'check_floor', target_floor=to_floor):
            return False
        if not self.call(
                self.exit_, 'exit_elevator', target_floor=to_floor,
                extra={'exit_target_distance_cm': 70.0}):
            return False
    ...
    def run(self):
        ...
        if not self.call(
                self.nav, 'go_to', TARGET_NAME, TARGET_FLOOR,
                extra={'start_delay_sec': 3.0}):
        ...
        if not self.call(
                self.nav, 'go_to', START_NAME, START_FLOOR,
                extra={'start_delay_sec': 3.0}):
