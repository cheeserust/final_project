...
# mission_manager의 legacy location 이름을 robot-side nav_points 키로 정규화한다.
# 실제 좌표는 계속 nav_points.yaml 한 곳에서만 소유한다.
TARGET_ALIASES = {
    '4': {
        'dock_4f': 'dock',
        'home_4f': 'home',
        'elevator_front_4f': 'elevator_front',
        'room_401': '401',
        'room_402': '402',
        'object': '402',
        'pickup_zone': '402',
    ...
    '5': {
        'dock_5f': 'dock',
        'elevator_front_5f': 'elevator_front',
        'room_501': '501',
        'object_place_5f': 'object_place',
    },
}
...
class NavGoToServer(Node):
    ...
    def make_pose(self, floor, target_name):
        ...
        if resolved_target not in floor_points:
            resolved_target = TARGET_ALIASES.get(floor_str, {}).get(
                target_name,
                target_name,
            )
        ...
        if resolved_target != target_name:
            self.get_logger().info(
                f'nav target alias: {target_name} -> {resolved_target}'
            )

        p = floor_points[resolved_target]
