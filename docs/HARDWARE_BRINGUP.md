...
MoveIt2 / CLI / Mission Manager
        |
        | ROS 2 ExecuteArmGoal / Board3 FollowJointTrajectory / Service
        v
arm_can_bridge
        |
        | SocketCAN can0
...
CAN bus
        |
        +-- Board1: arm_joint_1 ~ arm_joint_3 + base_joint
        +-- Board2: arm_joint_4
...
ros2 launch arm_can_bridge arm_can_bridge.launch.py \
  can_interface:=can0 execution_mode:=hardware
```
...
ros2 launch central_bringup arm_hardware_bringup.launch.py \
  can_interface:=can0 execution_mode:=hardware use_rviz:=false
...
## 9. PC/Raspberry Pi 분산 구성

이 프로젝트에서는 카메라 처리는 RPi에 두고 MoveIt, task executor,
`arm_can_bridge`, USB-CAN은 중앙 PC에 둔다. PC는 원본 영상 대신 원자적인
`/detected_marker` 메시지만 받는다.
...
실제 arm 실행 경로는 중앙 PC 안에서 CAN까지 이어진다.

```text
task_executor -> arm_can_bridge -> can0 -> STM32
```

RPi에서 PC로 넘어가는 것은 marker ID, pose, 원본 image timestamp다.
...
`/arm_controller/execute_joint_goal`이 보여야 한다. Board1/2 양쪽의 V3
capability probe가 성공하기 전에는 arm goal이 거절되는 것이 정상이다.

### 9.4 지연시간 최소화 체크리스트

1. PC와 Raspberry Pi를 유선 Ethernet으로 연결한다.
2. Raspberry Pi에서 RViz, 브라우저, MoveIt을 돌리지 않는다.
3. Raspberry Pi에서는 RealSense와 ArUco detector만 실행한다.
4. CAN을 쓰는 `arm_can_bridge`는 USB-CAN이 연결된 중앙 PC에서 돌린다.
...
9. RPi와 PC의 시간을 동기화해 timestamp 기반 TF 조회를 안정화한다.
...
Board1 V3 goal `0x101`은 local motor `0 -> 1 -> 2 -> 3`
...
```text
Wi-Fi -> 유선 LAN
Raspberry Pi에서 RViz/브라우저/MoveIt 종료
...
```text
1. MoveIt2 planning과 task executor는 PC에 둔다.
2. arm_can_bridge와 USB-CAN도 PC에 둔다.
3. RPi에는 RealSense와 ArUco detector만 둔다.
4. 원본 영상은 PC로 전송하지 않는다.
5. CAN frame 송신 간격은 PC에서 candump로 실제 측정한다.
```
