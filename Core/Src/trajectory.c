...
static uint8_t is_receiving_pending_command;  // 수신 진행 중
...
static uint8_t push_move_queue(const MultiAxisMoveCommand *command)
{
...
static uint8_t pop_move_queue(MultiAxisMoveCommand *command)
{
...
static void clear_pending_command(void)
{
    is_receiving_pending_command = 0;  // 수신 상태 초기화
...
{
    clear_pending_command();  // 수신 중이던 다축 이동 명령 취소
}
...
void trajectory_clear(void)
{
    ...
    clear_pending_command();  // 수신 중인 명령도 함께 초기화
...
static uint8_t resolve_target_step_from_cmd(uint8_t axis_id, int32_t target_pos, uint8_t flags, int32_t *target_step)
{
    uint8_t is_relative_move = (flags & 0b0010) ? 1 : 0;
    uint8_t is_step_mode = (flags & 0b0001) ? 1 : 0;
    int64_t resolved;
    int32_t min_step;
    int32_t max_step;
    ...
    resolved = is_step_mode ? target_pos : angle_to_step(axis_id, target_pos);
    if (is_relative_move) resolved += axis[axis_id].current_step;
...
{
    if (!is_receiving_pending_command) return 0;  // 수신 중인 명령이 없으면 정상
    if ((global_tick_ms - pending_start_ms) <= PENDING_TIMEOUT_MS) return 0;  // 제한 시간 이내면 계속 대기

    clear_pending_command();            // 시간 초과된 명령 취소
...
{
    uint8_t is_execute_requested = (command->flags & 0b1000) ? 1 : 0;  // 실행 플래그
    uint8_t is_relative_move = (command->flags & 0b0100) ? 1 : 0;      // 상대좌표 플래그
    uint8_t is_step_mode = (command->flags & 0b0010) ? 1 : 0;          // 스텝 직접 명령 플래그
    uint8_t has_reserved_flag = (command->flags & 0b0001) ? 1 : 0;     // 예약 플래그

    if (trajectory_handle_pending_timeout()) return TRAJECTORY_PENDING_INVALID;  // 이전 명령 수신이 시간 초과되면 실패

    // 지원 안하는 플래그 or 예약 플래그 or 축 번호 다르면 실패
    if (!is_execute_requested || has_reserved_flag || command->motor_id >= AXIS_COUNT) {
        clear_pending_command(); 
        return TRAJECTORY_PENDING_INVALID;
    }

    // 각도 벗어나면 실패
    if (!is_relative_move && !is_step_mode && (command->target_pos < angle_min[command->motor_id] || command->target_pos > angle_max[command->motor_id])) {
        clear_pending_command();
        return TRAJECTORY_PENDING_INVALID;
    }
    ...
    // 축 명령 받기!!!!!!!!!!
    if (!is_receiving_pending_command) {
        // 처음 모터 id가 0이 아니라면 실패, 0번부터 시작해야함
        if (command->motor_id != 0) 
        {
            return TRAJECTORY_PENDING_INVALID;
        }
        
        clear_pending_command();  // 새 수신 시작 전 상태 초기화
        is_receiving_pending_command = 1;  // 수신 시작
        pending_start_ms = global_tick_ms;  // 시작 시간 저장
        pending_command.move_duration_units = command->move_duration_units_from_can;  // 이동할 시간 지정
        ...
        // 축별 이동 시간이 다르면 실패
        if (command->move_duration_units_from_can != pending_command.move_duration_units) {
            clear_pending_command();
            return TRAJECTORY_PENDING_INVALID;
        }
    }
    ...
    if (!push_move_queue(&pending_command)) {
        clear_pending_command();  // 큐 입력 실패 시 수신 상태 초기화
        return TRAJECTORY_PENDING_QUEUE_FULL;  // 큐 가득 참
    }

    clear_pending_command();  // 큐에 넣은 뒤 수신 상태 초기화
...
static void try_start_next_command(void)
{
    ...
    if (!pop_move_queue(&command)) return;  // 대기 중인 명령이 없으면 종료

    for (uint8_t i = 0; i < AXIS_COUNT; i++) {
        if (!resolve_target_step_from_cmd(i, command.target_pos[i], command.flags[i], &target_step[i])) {
            global_motor_error = ERR_INVALID_CMD;
            return;
        }
    }

    move_duration = (uint16_t)command.move_duration_units * 5;  // 5ms 단위를 실제 ms로 변환
    if (move_duration == 0) move_duration = 1;  // 0ms 명령은 최소 1ms로 보정
    ...
    for (uint8_t i = 0; i < AXIS_COUNT; i++) {
        ...
        axis[i].move_total_time_ms = move_duration;    // 이동 총 실행 시간
