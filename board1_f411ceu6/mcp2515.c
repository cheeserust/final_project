...
{
    /* MCP2515 INT is active-low on PA10. This is used as a level fallback when
     * the falling-edge EXTI was missed or the INT line is already low.
     */
    return ((GPIOA->IDR & (1U << 10)) == 0U) ? 1 : 0;
}
...
static void mcp_exti_pa10_init(void)
{
    /* MCP2515 INT -> PA10, active-low falling edge */
    ...
    /* PA10 input + pull-up */
    Macro_Write_Block(GPIOA->MODER, 0x3, 0x0, 20);
    Macro_Write_Block(GPIOA->PUPDR, 0x3, 0x1, 20);

    /* EXTI10 source = PA10. EXTICR[2], bits 11:8 = 0 for port A */
    Macro_Write_Block(SYSCFG->EXTICR[2], 0xF, 0x0, 8);

    Macro_Set_Bit(EXTI->FTSR, 10);
    Macro_Clear_Bit(EXTI->RTSR, 10);
    EXTI->PR = (1U << 10);
