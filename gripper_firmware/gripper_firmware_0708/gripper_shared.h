...
// 모터 백오프(충돌 감지)를 위한 기본 부하 임계값 (0~1023)
#define GRIPPER_DEFAULT_LOAD_THRESHOLD    700U
/* ============================================================================
...
// [여기 추가] 그리퍼가 물건을 쥐고 멈춘 상태
#define STATE_CONTACT_HOLD                7U
...
typedef struct
{
    ...
    // 새로 추가한 부분 (모터별 부하 임계값 저장)
    uint16_t load_threshold[GRIPPER_MOTOR_COUNT];
    // 새로 추가 부하 제어 ON/OFF 스위치
    uint8_t load_control_enabled;
    /*
     * duration_5ms
     *
...
typedef struct
{
    ...
    /*
     * 추가: motor_status 303에서 사용
     * 각 모터별 2-bit 상태값 저장 (0: OK, 1: MOVING, 2: CONTACT_HOLD, 3: ERROR)
     */
    uint8_t motor_status[GRIPPER_MOTOR_COUNT];
