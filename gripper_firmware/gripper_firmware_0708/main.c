...
#include "gripper_mapping.h"     // 각도/스텝 맵핑 함수
#include "gripper_control.h"    // 그리퍼 제어 함수
...
typedef struct
{
    ...
    uint16_t load_threshold; // 기존 speed를 load_threshold로 변경
...
volatile uint32_t g_ms_tick = 0U;  // static 삭제
...
// 보드 초기화 함수, PC 디버깅, 모터 통신 준비 완료
static void Sys_Init(int debug_baud, int motor_baud) 
{
    // 1. 기존 필수 설정들 (그대로 유지)
    ...
    // 2. 통신 포트 2개 초기화
    Uart2_Init(debug_baud); // PC 화면 출력용 (예: 115200)
    Uart1_Init(motor_baud); // 서보모터 제어용 (예: 1000000)
    ...
    ///////테스트용 스위치 코드 /////////
    Key_Poll_Init(); // ★ 보드 스위치/버튼 초기화 (device_driver.h에 정의됨)
}
...
static void staging_clear(void)
{
    ...
    for (i = 0U; i < GRIPPER_MOTOR_COUNT; i++)
    {
        ...
        g_staging[i].load_threshold = 0U; // speed 대신 부하 한계값 적용
...
static void send_status(void)
{
    ...
    // 100ms 주기 및 각종 상태 변환마다 출력되므로 통신 지연을 막기 위해 주석 처리
...
static void send_position_feedback_group(uint8_t group_index, uint8_t base_motor_id)
{
    CAN_Frame_t pf;
    int16_t p0, p1, p2;
    uint8_t s0, s1, s2; // 상태값을 받을 변수 추가

    __disable_irq(); // 인터럽트 비활성화 (데이터 꼬임 방지)
    ...
    // 모터별 2-bit 상태값 안전하게 복사
    s0 = g_state.motor_status[base_motor_id + 0U];
    s1 = g_state.motor_status[base_motor_id + 1U];
    s2 = g_state.motor_status[base_motor_id + 2U];
    __enable_irq();
    ...
    // --- Byte7 비트 패킹 적용 ---
    uint8_t valid = 1; // 1로 고정
    uint8_t fault = 0;

    // 3개 모터 중 하나라도 진짜 에러(11, 즉 숫자 3)라면 그룹 fault를 1로 세움
    // CONTACT_HOLD(10, 숫자 2)는 에러로 치지 않음
    if (s0 == 3 || s1 == 3 || s2 == 3) {
        fault = 1;
    }

    // 비트 시프트(<<) 및 OR(|) 연산으로 데이터 조립
    pf.data[7] = (s0 & 0x03) |
                 ((s1 & 0x03) << 2) |
                 ((s2 & 0x03) << 4) |
                 (valid << 6) |
...
static void handle_enable_disable(const CAN_Frame_t *frame)
{
    ...
    if (cmd == 1U)
    {
        ...
        g_state.ready = 1U; // [추가] 준비 완료 상태 복구
        __enable_irq();
...
static void handle_gripper_home(const CAN_Frame_t *frame)
{
    ...
    // [여기 추가] 0x023 홈 명령이므로 부하 감지 끄기 (쫙 펴지도록)
    g_cmd.load_control_enabled = 0U;
...
static void handle_clear_error(const CAN_Frame_t *frame)
{
    ...
    g_state.fault = 0U;
    g_state.fault_motor_id = GRIPPER_NO_FAULT_MOTOR;
    g_state.ready = 1U; // [추가] 에러 해제 시 준비 완료 상태 복구
...
static void commit_staging_to_g_cmd(void)
{
    ...
    for (i = 0U; i < GRIPPER_MOTOR_COUNT; i++)
    {
        ...
        // 추가: 파싱 완료된 부하값을 제어단이 볼 수 있게 넘겨줌
        g_cmd.load_threshold[i] = g_staging[i].load_threshold;
    }
    g_cmd.duration_5ms = common_duration;

    g_cmd.load_control_enabled = 1U;  // [여기 추가] 0x103 명령이므로 부하 방어 로직 활성화
    g_cmd.is_new_cmd = 1U;
    g_state.state = STATE_MOVING;
    g_state.error_code = ERR_NONE;
    __enable_irq();
...
static void handle_gripper_command(const CAN_Frame_t *frame)
{
    ...
    uint8_t motor_id;
    int32_t target_pos;
    uint16_t load_threshold;  // 변수명 변경 (speed -> load_threshold)
    uint8_t duration_5ms;
    uint8_t count;
    ...
    // 서버가 보낸 부하값을 읽어오기
    load_threshold = read_u16_le(&frame->data[5]);
    if (load_threshold == 0) {
        load_threshold = GRIPPER_DEFAULT_LOAD_THRESHOLD; // 서버가 0보내면 기본값(예: 700)으로 강제 덮어쓰기
    }
    ...
    g_staging[motor_id].load_threshold = load_threshold;
    g_staging[motor_id].duration_5ms = duration_5ms;
    g_staging[motor_id].valid = 1U;

    update_staging_status();

    //printf("CMD staged motor=%u target_001deg=%ld load_threshold=%u duration_5ms=%u count=%u free=%u\n",
    //       motor_id,
    //       (long)target_pos,
    //       load_threshold,
    //       duration_5ms,
    ...
    // else 프레임 1개가 들어올 때마다 상태를 보고하면 CAN 버스와 SPI 통신이 낭비되므로 제거
    //{
    //    request_status_event();
    //}
}
...
// ---------------------------------------------------------
// [공유 전역 변수 초기화]
// 팀원의 CAN 수신/송신부와 함께 사용할 메모리 공간
// ---------------------------------------------------------
volatile GripperCommand g_cmd;
volatile GripperState   g_state;

// 테스트용 상태 변수 (0: 열림, 1: 닫힘)/////////
int test_step = 0;  //스위치로 테스트용
volatile int Key_Pressed = 0; // 스위치로 테스트용 변수
// volatile int current_mode = 0; // 줄을 추가해서 실체를 만드는 용
...
void Main(void)
{
...

    {
        ...
        for (;;)
        {
            ...
            // Gripper_Control_Task(); // 실제 모터 제어 함수로 대체

            // ★ [수정됨] 10ms(100Hz) 주기로만 모터 제어 및 피드백 읽기 수행 (통신 과부하 방지)
            // 프레임을 모으는 중(g_staging_active)이 아닐 때만 서보 제어 및 읽기 수행
            static uint32_t last_ctrl_ms = 0;
            if (!g_staging_active && (g_ms_tick - last_ctrl_ms >= 10U)) 
            {
                last_ctrl_ms = g_ms_tick;
                Gripper_Control_Task(); // 실제 모터 제어 및 피드백 업데이트 함수
            }
...
{
    Sys_Init(115200, 1000000); 
    printf("\nVicPinky Board3 Gripper Ready - Local Protection ON\n");
    shared_init();
    // ★ [추가] 1ms 타이머를 켜주어야 g_ms_tick 시간이 정상적으로 흐릅니다!
    tim3_1ms_init();
    g_state.enabled = 1;
    g_state.state = STATE_IDLE;
    g_state.error_code = ERR_NONE;
    ...
    TIM2_Delay(1000);  //오류 방지 시간지연 

    ////////////// 테스트용 제어코드 //////////////////
    for(;;)
    {
        // 1. 보드 스위치(Key)를 누를 때마다 가짜 명령 주입!
        if (Key_Get_Pressed() == 1) {
            Inject_Mock_Command(test_step);

            // 테스트 모드에서는 명령 주입 후 현재 위치를 바로 확인하자!
            printf("\n--- 현재 각 모터 위치 피드백 ---\n");
            for (int i = 0; i < GRIPPER_MOTOR_COUNT; i++) {
                uint8_t real_id = GRIPPER_MOTOR_ID_TO_SERVO_ID[i];
                // g_state.current_pos는 Gripper_Control_Task가 
                // 실시간으로 갱신해두고 있으니 이걸 바로 출력하면 됨!
                printf("모터ID %d: %ld 스텝\n", real_id, g_state.current_pos[i]);
                //  통신부가 가져갈 변환된 각도(0.01도 단위)도 함께 출력
                printf("모터ID %d: %ld 스텝 | 통신부 전달용 변환 각도: %d\n", 
               real_id, 
               g_state.current_pos[i], 
               g_state.current_pos_001deg[i]);
            }
            printf("-------------------------------\n");
            
            // 다음 테스트를 위해 상태 반전 (0 -> 1 -> 0)
            // test_step = (test_step == 0) ? 1 : 0; 
            ...
            // 스위치에서 손을 뗄 때까지 대기 (연속 눌림 방지 디바운스)
            Key_Wait_Key_Released(); 
        }

        // 2. 모터 제어 및 모니터링 루프 실행
